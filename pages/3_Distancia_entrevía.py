...
st.markdown ("# Distancia entrevía")
#st.sidebar.markdown(" # Distancia entrevía")
...
d2 = st.number_input("Peralte segunda vía (mm):",format='%.3f')

### Insuficiencia de peralte

i2 = st.number_input("Insuficiencia de peralte segunda vía (mm):",format='%.3f')

###Situación tren vía interior (circulando o parado) (Vía interior = Vía 1)
via_interior = st.selectbox("Estado tren vía 1:",["Circulando", "Parado"])

###Situación tren vía exterior (circulando o parado) (Vía exterior = Vía 2)
via_exterior = st.selectbox("Estado tren vía 2:",["Circulando", "Parado"])

via_interior = via_interior.lower()
via_exterior = via_exterior.lower()
...
def sum_EA(h,v,x1,x2):
    ...
    if st.session_state.ancho_via == 1.435:
        ...
        L = 1.055
    if h > hc0:
        sum_EA = K(h)*(Tvia()**2+(h+s0(h)*(h-hc0))**2*(TD(v)/L)**2+(np.tan(alpha_susp)**2+np.tan(alpha_c)**2+np.tan(alpha_osc(h,x1))**2)*(h-hc0)**2
                    +Tvia()**2+(h+s0(h)*(h-hc0))**2*(TD(v)/L)**2+(np.tan(alpha_susp)**2+np.tan(alpha_c)**2+np.tan(alpha_osc(h,x2))**2)*(h-hc0)**2)**0.5
    else:
        sum_EA = K(h)*(Tvia()**2+(h*TD(v)/L)**2+Tvia()+(h*TD(v)/L)**2)**0.5
    return sum_EA

def EA(h,v,R,D1,I1,D2,I2):
    sum_bD = bD(h,D1,D2)
    if via_interior == 'circulando' and via_exterior == 'circulando':
        EA = (st.session_state.bCR+Si_a(h,R)[0]+qSIi_a(h,I1)[0])+(st.session_state.bCR+Si_a(h,R)[1]+qSIi_a(h,I2)[1])+sum_bD+sum_EA(h,v,'interior','exterior')
    elif via_interior == 'parado' and via_exterior == 'parado':
        EA = (st.session_state.bCR+Si_a(h,R)[0]+qSDi_a(h,D1)[0])+(st.session_state.bCR+Si_a(h,R)[1]+qSDi_a(h,D2)[1])+sum_bD+sum_EA(h,v,'interior','exterior')
    elif via_interior == 'circulando' and via_exterior == 'parado':
        EA = (st.session_state.bCR+Si_a(h,R)[0]+qSIi_a(h,I1)[0])+(st.session_state.bCR+Si_a(h,R)[1]+qSDi_a(h,D2)[1])+sum_bD+sum_EA(h,v,'interior','exterior')
    elif via_interior == 'parado' and via_exterior == 'circulando':
        EA = (st.session_state.bCR+Si_a(h,R)[0]+qSDi_a(h,D1)[0])+(st.session_state.bCR+Si_a(h,R)[1]+qSIi_a(h,I2)[1])+sum_bD+sum_EA(h,v,'interior','exterior')
    return EA


distancia_entreeje = EA(st.session_state.hPT_max,st.session_state.v,st.session_state.r,st.session_state.d,st.session_state.i,d2,i2)
st.write('La distancia entreeje mínima requerida es %s metros.' % (distancia_entreeje.round(2)))